...
def get_non_python_libs():
    ...
    Tabs on current platform.
    Every pair corresponds to a single library file.
    First tuple item is path in local filesystem during build.
    Second tuple item correspond to path expected in Tabs installation
    relative to Tabs prefix.
...
registry_table = [
    ('TabsKLM', 2, r'SOFTWARE\Tabs', '*', None, 'TARGETDIR'),
    ('TabsInstallDir', 2, r'SOFTWARE\Tabs', 'InstallDir', '[TARGETDIR]', 'TARGETDIR'),
    ('TabsExecutable', 2, r'SOFTWARE\Tabs', 'Executable', '[TARGETDIR]Tabs.exe', 'TARGETDIR'),
]

# Provide the locator and app search to give MSI the existing install directory
# for future upgrades
reg_locator_table = [
    ('TabsInstallDirLocate', 2, r'SOFTWARE\Tabs', 'InstallDir', 0),
]
app_search_table = [('TARGETDIR', 'TabsInstallDirLocate')]
...
bdist_msi_options = {
    ...
    "all_users": True,
    "add_to_path": True,
    "install_icon": "data/icons/org.gnome.Tabs.ico",
}

executable_options = {
    "script": "bin/Tabs",
    "icon": "data/icons/org.gnome.Tabs.ico",
}
...
if 'mingw' in sysconfig.get_platform():
    executable_options.update({
        ...
        "target_name": "Tabs.exe",
        "shortcut_name": "Tabs",
        "shortcut_dir": "ProgramMenuFolder",
    })
    console_executable_options.update({
        "target_name": "TabsConsole.exe",
    })

# Copy conf.py in place if necessary
base_path = pathlib.Path(__file__).parent
conf_path = base_path / 'Tabs' / 'conf.py'

if not conf_path.exists():
    import shutil
    shutil.copyfile(conf_path.with_suffix('.py.in'), conf_path)

import Tabs.build_helpers  # noqa: E402
import Tabs.conf  # noqa: E402

setup(
    name="Tabs",
    version=Tabs.conf.__version__,
    description='Look at tabulatures in style 🎸',
    maintainer='Zoey Ahmed',
    url='https://zoey.blahaj.land/',
    license='GPLv2+',
    classifiers=[
    ...
    package_data={
        'Tabs': ['README', 'COPYING', 'NEWS'],
        'Tabs.vc': ['README', 'COPYING'],
    },
    scripts=['bin/Tabs'],
    data_files=[
        ('share/man/man1',
        ...
        ('share/doc/Tabs-' + Tabs.conf.__version__,
         ['COPYING', 'NEWS']
         ),
        ('share/Tabs/icons',
    ...
    cmdclass={
        "build_i18n": Tabs.build_helpers.build_i18n,
        "build_help": Tabs.build_helpers.build_help,
        "build_icons": Tabs.build_helpers.build_icons,
        "build_data": Tabs.build_helpers.build_data,
    }
)
